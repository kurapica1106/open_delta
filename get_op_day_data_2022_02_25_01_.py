...
def op_df(y, m, d):
    ...
    if dfs_check.iat[1, 0] != "查無資料":
        ...
        for i in range(len(data_frames)-2):
            df.append(
                [y, m, d,
                 data_frames.iat[i, 1], data_frames.iat[i, 2], data_frames.iat[i, 3],
                 data_frames.iat[i, 4], data_frames.iat[i, 5], data_frames.iat[i, 6], data_frames.iat[i, 7],
                 data_frames.iat[i, 12], data_frames.iat[i, 14]
                 ]
                )
    return df
...
def write_to_csv(y1, m1, d1, y2, m2, d2):
    with open("op_day_data.csv", "w", newline = "") as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["Year", "Month", "Day",
                             "Contract", "Strike Price", "P//C",
                             "O", "H", "L", "C", "V", "OI"])
